...
Developer-facing contract for what the MarkdownEditor SHALL expose to make its behaviors testable: a headless-editor surface for pure transformer / helper tests, a Lexical-command surface for plugin-keyboard tests driven inside a real browser, and a shared test-utility module. Locks in the testability promise so future plugins are written with tests in mind, not bolted on after.
