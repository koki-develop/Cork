...
ユーザがタスクのタグでボードを絞り込めるようにする。6 種のオペレータ (`contains` / `not_contains` / `contains_any` / `contains_all` / `is_empty` / `is_not_empty`) を全体 AND で評価し、既存のタイトル検索 (`search-tasks`) とも AND で組み合わさる。フィルタの編集はツールバーの Filter popover で行い、ワークスペース毎にローカル永続化する。バックエンドはタスクキャッシュに対してフィルタを in-memory で評価するため、ファイル I/O は発生しない。

`TagFilter` 型は operator を判別子とした discriminated union で、タグを取らない operator (`is_empty` / `is_not_empty`) は `tags` フィールドを持たない。これにより「`is_empty` + 非空タグ」のような不正な状態を型レベルで表現不能にしている。
