...
**検討した代替案:**
- *全部 `onChange` で即時保存*: ステータスラベルは打鍵ごとに `save_statuses` が走り、ストアへの書き込みが多発する。ユーザー体験的にも「タイポ途中の状態が永続化される」のは違和感がある → 却下
- *デバウンス（300ms など）*: タイマー管理が増え、フォーカスアウト時に未保存のまま閉じるエッジケースが残る → 却下。`onBlur` で十分
