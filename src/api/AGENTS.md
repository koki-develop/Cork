...
- `workspaceName.ts` — `getWorkspaceName`, `setWorkspaceName`. Persisted in `.cork.json` (like `statuses.ts`), not `settings.json` — a separate backend module (`workspace_name.rs`) from `workspace.ts`'s Rust counterpart despite the similar name.
- `mcp.ts` — Embedded MCP server (process-global, not per-window): `getMcpSettings`, `updateMcpSettings`, `generateMcpToken`, `getMcpSampleConfig`, `getMcpSetupSnippets` (per-tool setup snippets for Claude Code / Codex CLI / opencode), `getMcpServerStatus`, and `onMcpSettingsChange` (subscription wrapper around the `tauri-plugin-store` `store://change` event filtered to the `mcp` key — replaces frontend polling for cross-window sync). Settings persist under the top-level `mcp` key of `settings.json`.
- `menu.ts` — Native menu events: `onOpenSettings` (subscription to `menu:open-settings`), `onOpenCreateTask` (subscription to `menu:open-create-task`), and `onCheckForUpdates` (subscription to `menu:check-for-updates`).
- `updater.ts` — In-app updater wiring. Thin wrappers over `@tauri-apps/plugin-updater` (`checkForUpdate`, `downloadAndInstall`) and `@tauri-apps/plugin-process` (`relaunchApp`). Cork keeps no persisted updater settings of its own — auto-check is always on — so there are no Rust commands behind this; the file's job is only to confine the `@tauri-apps/plugin-updater` import to `src/api/`. The `Update` type and `DownloadProgress` event shape are re-exported for `useUpdater` to consume.
