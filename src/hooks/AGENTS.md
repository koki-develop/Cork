...
- `useMcpSettings.ts` — Process-global MCP server settings + live status. Event-driven: subscribes to `onMcpSettingsChange` (Tauri `store://change` broadcast filtered to the `mcp` key) for the hook's lifetime, so any window's write to `settings.json` immediately refreshes this window's UI. Re-fetches on dialog open to catch AppState-only changes that don't emit a store event (e.g. another window opening a workspace, which affects the sample mcp.json and the per-tool setup snippets — both `sampleConfig` and `setupSnippets` are pulled in the same `refresh`). All mutations (`updateEnabled` / `updateToken` / `regenerateToken`) persist immediately — no debounce. A `loaded` flag gates mutations until the initial fetch resolves so the placeholder token can't slip into a backend save.
- `useUpdater.ts` — In-app updater state machine. Discriminated state: `idle | checking | available | downloading | installing | error`. Auto-check always runs on mount (no opt-out), but **only when `getCurrentWebviewWindow().label === "main"`** so the `workspace-<n>` Windows naturally skip it (`main` is the reserved label of every process's first Window per `lib.rs::MAIN_WINDOW_LABEL`). A module-level `autoCheckFiredOnce` ref additionally guards React 18 StrictMode double-mount in dev. Manual check (`Cork > Check for Updates...` menu) runs through the same `runCheck` with `mode: "manual"`; the difference is the manual path surfaces a `Cork is up to date.` info toast on null results (auto stays silent to avoid every-launch noise). Owns the `installAndRestart` flow: subscribes to the plugin's progress callback (Started → Progress → Finished), transitions state, then calls `relaunchApp()`. The UI lives in `organisms/shell/UpdaterToast`, which subscribes to the state and bridges to sonner; `useUpdater` itself is UI-agnostic.
...
- `useReanchorOnScroll.ts` — Re-invokes a callback on window scroll (capture phase) or resize while active. Shared wiring behind any floating panel that tracks a viewport-relative anchor; used by `FloatingLinkEditorPlugin` and `FloatingCodeLanguageEditorPlugin`, both of which own their own "is the anchor still valid, recompute it" `reanchor` logic and just hand this hook the callback.
