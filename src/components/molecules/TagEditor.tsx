  ...
  function TagEditor(
    ...
    const filteredSuggestions = useMemo(() => {
      ...
      return suggestions.filter(
        (s) => !tags.includes(s) && fuzzySubsequenceMatch(s, pending),
      );
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: container size depends on tags/pending, recompute popover position when they change
    useLayoutEffect(() => {
    ...
    return (
      <>
        <div
          ref={containerRef}
          className={clsx(
            "relative flex min-h-[34px] flex-wrap items-center gap-1.5 rounded-lg border border-cork-border/40 bg-cork-elevated/60 px-2 py-1.5",
        ...
        >
          ...
          <input
            ...
            disabled={isFull}
            className="h-5 min-w-[60px] flex-1 border-none bg-transparent px-1 text-cork-text text-xs outline-none placeholder:text-cork-muted/50 disabled:cursor-not-allowed"
          />
        </div>
        {createPortal(
          <AnimatePresence>
            {suggestionsEnabled &&
              ...
              suggestionPos && (
                <m.div
                  ...
                  className="fixed z-[60] max-h-[200px] origin-top-left overflow-y-auto rounded-lg border border-cork-border/60 bg-cork-surface text-xs shadow-2xl"
