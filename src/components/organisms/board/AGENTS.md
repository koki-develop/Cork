...
Both dialogs are `max-w-4xl` modals laid out as a single 2-column row: a `md:flex-1` main column (Title + Body) beside a `md:w-60` sidebar (`md:gap-6`), stacking vertically below the `md` breakpoint. `CreateTaskDialog` opens with a static `DialogHeader` ("New Task"), then the 2-column row holding Title + Body (left) and Status + Tags (right). Its Cancel / Create footer lives at the bottom of that right sidebar (pinned with `md:mt-auto`) rather than as a full-width row beneath the columns: right-aligned it lands in the same bottom-right corner, but since it no longer forms a row under the Body, the Body fills straight down to the action row instead of leaving an empty band beneath it. `TaskDetailDialog` has no static heading — its editable Title doubles as the heading and leads the left column above the Body. Its right sidebar leads with the overflow-menu + close chrome (right-aligned in the dialog's top-right corner), then Status + Tags below. Because both columns are top-aligned and the sidebar is independent of the Title's height, a Title that wraps to several lines only pushes the Body down — Status and Tags stay pinned to the top instead of drifting down with it. The shared `md:flex-1` / `md:w-60` widths keep the Title's right edge lined up with the Body's.
