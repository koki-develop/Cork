...
- `BoardPage.tsx` — The kanban view. The orchestration hub:
  ...
  - **Owner of** `settingsOpen`, the `menu:open-settings` subscription (native `Cmd+,`), the `menu:open-create-task` subscription (native `Cmd+N` → opens the create dialog), and the board-scoped `DragDropProvider`.
