...
- `filter.ts` — `TagFilterOperator` (the discriminator union), `TagOperatorWithTags` / `TagOperatorWithoutTags` (the two operator subsets), `TagFilter` (UI form with `id` for React keys), `StoredFilter` (the persisted form sent to the backend).
