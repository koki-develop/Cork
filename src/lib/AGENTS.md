...
- `date.ts` — Due-date helpers: `parseDate`, `formatISODate`, `classifyDate` (buckets a date as overdue/today/tomorrow/soon/far), `describeDate` (bucket + card label in one pass).
...
- `task.ts` — Task form diff: `TaskFormSnapshot`, `taskFormSnapshot` (seeds a snapshot from a fetched `Task`), `computeDirtyUpdates` (returns a sparse `TaskUpdates`), `withTaskUpdates`.
